...
/**
 * Consent is a launch gate, so it has to be visible at a glance: an
 * operator must be able to see WHY a lead cannot be called without
 * opening anything.
 */
...
/**
 * The setup checklist. Live calling is refused by the API until every
 * required item is complete, so the console has to say exactly what is
 * missing rather than letting the operator discover it when a call fails.
 */
function SetupChecklist({ compact = false }: { compact?: boolean }) {
  ...
  if (ready_for_live_calls && !outstanding.length) {
    ...
    return <section className="rounded-xl border border-[hsl(var(--accent)/.3)] bg-[hsl(var(--accent)/.06)] p-5" data-testid="panel-setup-ready">
      <div className="flex items-center gap-3"><CheckCircle2 className="text-[hsl(var(--accent))]" size={18} /><div><h3 className="font-bold">Setup complete — live calling is enabled</h3><p className="mt-0.5 text-xs text-muted-foreground">Every required item is configured. The safety policy still gates each individual call.</p></div></div>
    </section>;
  }
  return <section className={`rounded-xl border p-5 ${ready_for_live_calls ? 'border-border bg-[hsl(var(--card))]' : 'border-[hsl(var(--destructive)/.35)] bg-[hsl(var(--destructive)/.05)]'}`} data-testid="panel-setup-checklist">
    <div className="flex items-start gap-3">
      <div className={`rounded-lg p-2 ${ready_for_live_calls ? 'bg-[hsl(var(--muted))] text-muted-foreground' : 'bg-[hsl(var(--destructive)/.12)] text-[hsl(var(--destructive))]'}`}>{ready_for_live_calls ? <ShieldCheck size={18} /> : <ShieldAlert size={18} />}</div>
      <div className="flex-1">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h3 className="font-bold">{ready_for_live_calls ? 'Recommended setup still outstanding' : 'Live calling is disabled until setup is complete'}</h3>
          <Link href="/workspace/business-settings" className="text-xs font-bold text-[hsl(var(--accent))] hover:underline" data-testid="link-open-setup-settings">Open settings <ArrowUpRight size={13} className="ml-1 inline" /></Link>
        </div>
        {!ready_for_live_calls && <p className="mt-1 text-sm text-muted-foreground">{missing_required.length} required {missing_required.length === 1 ? 'item is' : 'items are'} missing. LeadSprint will not place calls until they are configured.</p>}
        <div className="mt-4 space-y-2">
          {outstanding.map((item) => <div key={item.key} className="flex items-start gap-2.5 text-sm" data-testid={`checklist-item-${item.key}`}>
            <span className={`mt-1 h-3.5 w-3.5 shrink-0 rounded-[4px] border ${item.severity === 'required' ? 'border-[hsl(var(--destructive)/.55)]' : 'border-border'}`} />
            <span className="min-w-0"><span className="font-semibold">{item.label}</span>{item.severity === 'recommended' && <span className="ml-2 text-[10px] font-bold uppercase tracking-[.1em] text-muted-foreground">optional</span>}<span className="mt-0.5 block text-xs leading-5 text-muted-foreground">{item.detail}</span></span>
          </div>)}
        </div>
        <p className="mt-4 text-xs text-muted-foreground">{items.length - outstanding.length} of {items.length} configured.</p>
      </div>
    </div>
  </section>;
}
...
function LeadsPage() {
  ...
  const handleImport = (event: any) => {
    ...
    // Consent is a launch gate: an import has to say where consent for
    // this batch came from, or every row lands uncallable.
    ...
    reader.onload = () => {
      // Proper RFC 4180 parsing — the previous split(',') silently shifted
      // every column after any quoted comma, which put the wrong value in
      // the phone field.
      const parsed = parseLeadCsv(String(reader.result || ''));
      if (parsed.error) { setImportError(parsed.error); return; }
      if (!parsed.rows.length) { setImportError('No rows in that file had both a name and a phone number.'); return; }
      if (parsed.skipped) setImportNotice(`${parsed.skipped} row${parsed.skipped === 1 ? '' : 's'} skipped for a missing name or phone number.`);
      importLeads.mutate(
        { data: { rows: parsed.rows as any, consent_status: 'valid', consent_source: consentSource.trim() } },
        { onSuccess: () => { setShowImport(false); setConsentSource(''); queryClient.invalidateQueries({ queryKey: getGetLeadsQueryKey() }); } },
      );
    };
  ...
  return <div className="animate-rise-in space-y-5">
    ...
    {leads.isLoading ? <div className="space-y-2">{[1, 2, 3, 4, 5].map((n) => <Skeleton key={n} className="h-20" />)}</div> : leads.isError ? <ErrorState retry={() => leads.refetch()} /> : !leads.data?.length ? <EmptyState icon={UsersRound} title="No leads match that view" description="Try clearing a filter or import a normalized CSV to start the desk." action={<Button onClick={() => { setSearch(''); setStatus('all'); setScore('all'); }} data-testid="button-clear-lead-filters">Clear filters</Button>} /> : <div className="overflow-hidden rounded-xl border border-border bg-[hsl(var(--card))]"><div className="hidden grid-cols-[minmax(220px,1.3fr)_1fr_1fr_1fr_120px] gap-4 border-b border-border bg-[hsl(var(--muted)/.55)] px-5 py-3 text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground md:grid"><span>Lead</span><span>Intent</span><span>Need</span><span>Next action</span><span /></div><div className="divide-y divide-border">{leads.data.map((lead) => <div key={lead.id} className="grid gap-3 px-4 py-4 transition hover:bg-[hsl(var(--muted)/.32)] md:grid-cols-[minmax(220px,1.3fr)_1fr_1fr_1fr_120px] md:items-center md:gap-4 md:px-5" data-testid={`row-lead-${lead.id}`}><button className="flex min-w-0 items-center gap-3 text-left" onClick={() => setSelected(lead.id)} data-testid={`button-open-lead-${lead.id}`}><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[hsl(var(--secondary)/.35)] font-mono text-[11px] font-bold text-[hsl(var(--primary))]">{initials(lead.name)}</span><span className="min-w-0"><span className="block truncate text-sm font-bold">{lead.name}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{lead.phone} · {lead.project}</span></span></button><div className="flex flex-wrap items-center gap-2"><Badge className={scoreTone(lead.score)}>{lead.score}</Badge><span className="font-mono text-xs text-muted-foreground">{lead.intent_score}/100</span>{lead.consent_status !== 'valid' && <ConsentBadge status={lead.consent_status} />}</div><div><p className="text-sm font-medium">{lead.property_type}</p><p className="mt-0.5 text-xs text-muted-foreground">{lead.budget_label} · {lead.timeline}</p></div><div><p className="text-sm font-medium">{lead.next_action || 'Review lead'}</p><p className="mt-0.5 text-xs text-muted-foreground">{relativeTime(lead.last_call || lead.created_at)}</p></div><div className="flex items-center justify-end gap-1"><button onClick={() => startCall.mutate({ data: { lead_id: lead.id } }, { onSuccess: () => queryClient.invalidateQueries({ queryKey: getGetCallsQueryKey() }) })} disabled={!lead.callable || startCall.isPending} className="rounded-lg p-2 text-[hsl(var(--accent))] hover:bg-[hsl(var(--accent)/.1)] disabled:opacity-40" title={lead.callable ? 'Call now' : lead.consent_detail || 'This lead cannot be called'} data-testid={`button-call-row-${lead.id}`}><Phone size={16} /></button><button onClick={() => setSelected(lead.id)} className="rounded-lg p-2 text-muted-foreground hover:bg-[hsl(var(--muted))]" title="Open details" data-testid={`button-details-row-${lead.id}`}><ChevronRight size={16} /></button></div></div>)}</div></div>}
    {selected && <LeadDetail id={selected} onClose={() => setSelected(null)} />}
    {showImport && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#102632]/35 p-5"><div className="w-full max-w-[440px] rounded-2xl border border-border bg-[hsl(var(--card))] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><h2 className="text-lg font-bold">Import lead rows</h2><p className="mt-1 text-sm text-muted-foreground">Use a CSV with name and phone columns. Quoted commas are handled.</p></div><button onClick={() => setShowImport(false)} data-testid="button-close-import"><X size={18} /></button></div><label className="mt-5 block text-sm font-semibold">Where was consent captured?<input value={consentSource} onChange={(event) => { setConsentSource(event.target.value); setImportError(null); }} placeholder="e.g. Website enquiry form, 12 Sep 2026" className="mt-2 w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm" data-testid="input-consent-source" /><span className="mt-1.5 block text-xs font-normal leading-5 text-muted-foreground">Required. Leads imported without recorded consent evidence are never called.</span></label><label className="mt-5 flex cursor-pointer flex-col items-center rounded-xl border border-dashed border-[hsl(var(--accent)/.5)] bg-[hsl(var(--accent)/.04)] p-8 text-center hover:bg-[hsl(var(--accent)/.08)]"><Upload className="text-[hsl(var(--accent))]" size={22} /><span className="mt-3 text-sm font-semibold">Choose CSV file</span><span className="mt-1 text-xs text-muted-foreground">Rows are normalized before import.</span><input type="file" accept=".csv,text/csv" onChange={handleImport} className="hidden" data-testid="input-import-csv" /></label>{importLeads.isPending && <p className="mt-3 text-xs text-muted-foreground">Importing rows…</p>}{importNotice && <p className="mt-3 text-xs text-muted-foreground" data-testid="text-import-notice">{importNotice}</p>}{(importError || importLeads.isError) && <p className="mt-3 text-xs text-[hsl(var(--destructive))]" data-testid="text-import-error">{importError || 'Import failed. Check the required columns and try again.'}</p>}<Button variant="quiet" className="mt-5 w-full" onClick={() => setShowImport(false)} data-testid="button-cancel-import">Cancel</Button></div></div>}
  </div>;
}
